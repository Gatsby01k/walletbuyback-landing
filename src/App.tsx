...
/* === i18n helpers for autodetect & persist (добавлено) === */
...
function getInitialLang(): Lang {
  ...
  const prefs = (navigator.languages && navigator.languages.length ? navigator.languages : [navigator.language]).filter(Boolean) as string[];
      ...
      <div className="relative z-10">
          ...
          <div className="mx-auto max-w-7xl px-4 py-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3 relative" ref={menuRef}>
              <nav className="hidden md:flex items-center gap-6 text-sm">
                ...
                <a href="#how" className="hover:opacity-80 transition">{t("nav_how")}</a>
                <a href="#features" className="hover:opacity-80 transition">{t("nav_features")}</a>
                <a href="#form" className="hover:opacity-80 transition">{t("nav_form")}</a>
              </nav>
              ...
              {/* === language select (добавлено) === */}
              <Select value={lang} onValueChange={(v) => setLang(v as Lang)}>
                <SelectTrigger className="w-[160px]">
                  <SelectValue placeholder="ru" />
                </SelectTrigger>
                <SelectContent className={`${isDark ? 'bg-[#0b1020] border-white/10 text-white' : 'bg-white border border-gray-200 text-gray-900'}`} align="end">
              ...
              <Magnetic enabled={enableFancy}><Button className={buttonPrimary} asChild><a href="#form">{t("hero_cta")}</a></Button></Magnetic>
          ...
          <div className="mx-auto max-w-7xl px-4 py-14 md:py-24 grid md:grid-cols-2 gap-10 md:gap-12 items-center">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-20% 0px" }} transition={{ duration: 0.6 }} className="space-y-6">
              <div className={`inline-flex items-center gap-2 rounded-full px-3 py-1 ring-1 ${isDark ? 'bg-white/10 ring-white/15' : 'bg-teal-50 ring-teal-200'}`}>
                ...
                <span className={`text-xs tracking-wide ${isDark ? 'text-white/80' : 'text-teal-700'}`}>{t("hero_badge_fast_estimate")} · +$5000</span>
              </div>
              <h1 className="text-3xl sm:text-5xl md:text-6xl font-bold leading-[1.05]">
                ...
                <Reveal delay={0.15}>{t("hero_title_and")} <motion.span whileHover={{ textShadow: `0 0 8px ${BRAND_COLORS.trust}` }} transition={{ type: 'spring', stiffness: 200, damping: 15 }} style={{ color: BRAND_COLORS.trust }}>Trust Wallet</motion.span></Reveal>
              </h1>
              ...
              <TiltCard enabled={enableFancy} className="will-change-transform">
                <Card className={`${cardChrome} rounded-3xl overflow-hidden shadow-sm`}>
                  ...
                  <CardContent className="grid gap-4">
                    <div className="grid gap-2">
                      ...
                      <Select value={network} onValueChange={setNetwork}>
                        <SelectTrigger className={inputChrome}><SelectValue placeholder={t("form_network_placeholder")} /></SelectTrigger>
                    ...
                    <div className="grid gap-2">
                      ...
                      <Input placeholder={t("form_address_placeholder")} value={address} onChange={(e) => setAddress(e.target.value)} className={inputChrome} />
                    </div>
                    <div className="grid gap-2">
                      <Label className={labelColor}>{t("form_contact_label")}</Label>
                      <Input placeholder={t("form_contact_placeholder")} value={contact} onChange={(e) => setContact(e.target.value)} className={inputChrome} />
                    </div>
                    <div className="grid gap-2">
                      <Label className={labelColor}>{t("form_desc_label")}</Label>
                      <Textarea placeholder={t("form_desc_placeholder")} value={note} onChange={(e) => setNote(e.target.value)} className={`${inputChrome} min-h-[96px]`} />
                    </div>
                  ...
                  <CardFooter className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div className={`text-xs flex items-center gap-2 ${fineText}`}><ShieldCheck className="h-4 w-4" /> {t("form_payouts_hint")}</div>
                    <Magnetic enabled={enableFancy}><Button onClick={handleSubmit} disabled={!canSubmit} className={buttonPrimary}><Send className="mr-2 h-4 w-4" /> {t("form_submit")}</Button></Magnetic>
                  </CardFooter>
                </Card>
              </TiltCard>
              {submitted && (<div className={`mt-4 text-sm ${isDark ? 'text-teal-300' : 'text-teal-700'}`}>{t("form_thanks")}</div>)}
            </motion.div>
          </div>
        </section>
        ...
        <section id="features" className="mx-auto max-w-7xl px-4 pb-8 md:pb-16">
          <div className="grid md:grid-cols-3 gap-6">
            {[
              { icon: <ShieldCheck className={`h-5 w-5 ${isDark ? 'text-teal-400' : 'text-teal-600'}`} />, title: t("feat_transparency_title"), text: t("feat_transparency_text") },
              { icon: <Wallet className={`h-5 w-5 ${isDark ? 'text-teal-400' : 'text-teal-600'}`} />, title: t("feat_networks_title"), text: t("feat_networks_text") },
              { icon: <Coins className={`h-5 w-5 ${isDark ? 'text-teal-400' : 'text-teal-600'}`} />, title: t("feat_fast_payout_title"), text: t("feat_fast_payout_text") },
            ].map((f, i) => (
        ...
        <section id="how" className="mx-auto max-w-7xl px-4 py-20">
          ...
          <div className="grid md:grid-cols-4 gap-6">
            {[
              { num: "01", title: t("step_01"),  text: t("step_01_text"), icon: <Wallet className={`${isDark ? 'text-teal-300' : 'text-teal-600'} h-5 w-5`} /> },
              { num: "02", title: t("step_02"), text: t("step_02_text"), icon: <Sparkles className={`${isDark ? 'text-teal-300' : 'text-teal-600'} h-5 w-5`} /> },
              { num: "03", title: t("step_03"),  text: t("step_03_text"), icon: <ShieldCheck className={`${isDark ? 'text-teal-300' : 'text-teal-600'} h-5 w-5`} /> },
              { num: "04", title: t("step_04"), text: t("step_04_text"), icon: <Coins className={`${isDark ? 'text-teal-300' : 'text-teal-600'} h-5 w-5`} /> },
            ].map((s, i, arr) => (
              <motion.div
                key={s.num}
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className={`text-xs uppercase tracking-wider ${fineText}`}>{t("step_label")} {s.num}</span>
                  </div>
          ...
          <div className="grid md:grid-cols-2 gap-8 md:gap-10 items-start">
            <div>
              ...
              <ul className={`space-y-3 text-sm ${fineText}`}>
                <li className="flex gap-2"><Lock className="h-4 w-4 mt-0.5"/> {t("seed_safety_hint")}</li>
                <li className="flex gap-2"><MessageSquare className="h-4 w-4 mt-0.5"/> {t("form_contacts_hint")}</li>
                <li className="flex gap-2"><ShieldCheck className="h-4 w-4 mt-0.5"/> {t("form_buyback_hint")}</li>
              </ul>
            </div>
            <form onSubmit={handleSubmit} className={`${cardChrome} rounded-3xl p-6 grid gap-4 shadow-sm`}>
              ...
              <Magnetic enabled={enableFancy}><Button type="submit" disabled={!computeCanSubmit(address, contact, agree)} className={buttonPrimary}><Send className="mr-2 h-4 w-4" /> {t("form_submit_short")}</Button></Magnetic>
              {submitted && (<div className={`${isDark ? 'text-teal-300' : 'text-teal-700'} text-sm`}>{t("form_thanks")}</div>)}
            </form>
          </div>
        </section>
        ...
        <footer className={`mt-4 border-t ${isDark ? 'border-white/10' : 'border-gray-200'}`}>
          <div className="mx-auto max-w-7xl px-4 py-8 grid md:grid-cols-2 gap-6 items-center">
            <p className={`text-xs ${fineText}`}>© {new Date().getFullYear()} WalletBuyBack — {t("footer_tagline")}</p>
            <div className="flex justify-start md:justify-end gap-4 text-sm">
              <a href="#how" className="hover:opacity-80">{t("nav_how")}</a>
              <a href="#features" className="hover:opacity-80">{t("nav_features")}</a>
              <a href="#form" className="hover:opacity-80">{t("nav_form")}</a>
            </div>
          </div>
        </footer>
      </div>
